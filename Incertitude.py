...
def create_dataset_folder(df, folder_name):
    ...
    if not os.path.exists(target_dir):
        ...
        print(f"Le dossier {folder_name} existe déjà, on pointe vers les fichiers existants.")
...
# ==========================================
# 4. DATASET & MODÈLE (CORRIGÉ POUR LE PTH)
# ==========================================
...
class GazeNet(nn.Module):
    ...
    def forward(self, x): 
        ...
        # Bruit MC Dropout appliqué à la volée (ne casse pas le chargement des poids)
        x = F.dropout(x, p=0.2, training=self.training)
        return self.regressor(x)

# ==========================================
# 5. FONCTIONS D'INCERTITUDE ET D'ÉVALUATION
# ==========================================
def get_contrast_uncertainty(model, pool_df):
    pool_loader = DataLoader(EyeTrackingDatasetB(pool_df), batch_size=32, shuffle=False)
    model.eval()
    uncertainties = []
    
    with torch.no_grad():
        for imgs, _ in tqdm(pool_loader, desc="Calcul Contraste", leave=False):
            imgs = imgs.to(DEVICE)
            imgs_low = TF.adjust_contrast(imgs, 0.5)
            imgs_high = TF.adjust_contrast(imgs, 1.5)
            
            preds_norm = model(imgs)
            preds_low = model(imgs_low)
            preds_high = model(imgs_high)
            
            diff1 = torch.norm(preds_norm - preds_low, dim=1)
            diff2 = torch.norm(preds_norm - preds_high, dim=1)
            diff3 = torch.norm(preds_low - preds_high, dim=1)
            max_diff = torch.max(torch.stack([diff1, diff2, diff3]), dim=0)[0]
            uncertainties.extend(max_diff.cpu().numpy())
    return np.array(uncertainties)

def get_mcdropout_uncertainty(model, pool_df, num_passes=10):
    pool_loader = DataLoader(EyeTrackingDatasetB(pool_df), batch_size=32, shuffle=False)
    model.train() # Active le F.dropout dans le forward
    uncertainties = []
    ...
    with torch.no_grad():
        for imgs, _ in tqdm(pool_loader, desc="Calcul MC Dropout", leave=False):
            ...
            for _ in range(num_passes):
                batch_preds.append(model(imgs))
                
            batch_preds = torch.stack(batch_preds)
            variance = torch.var(batch_preds, dim=0)
            score = torch.sum(variance, dim=1)
...
print("\n>>> Évaluation RANDOM (3 répétitions pour l'intervalle de confiance)")
...
# ------------------------------------------
# B. STRATÉGIE INCERTITUDE (CONTRASTE)
# ------------------------------------------
print("\n>>> Évaluation INCERTITUDE (Contraste)")
contrast_mae_history = []
df_pool = df_train_final.copy()
df_labeled = pd.DataFrame()
...
for i, step in enumerate(steps):
    ...
    if step > 0:
        n_to_add = step - steps[i-1]
        print(f"    Recherche de {n_to_add} images incertaines...")
        scores = get_contrast_uncertainty(model, df_pool)
        top_indices = np.argsort(scores)[::-1][:n_to_add]
        new_samples = df_pool.iloc[top_indices]
    ...
    mae = evaluate_model(model, test_loader, criterion)
    contrast_mae_history.append(mae)
    print(f"    Palier {step} -> MAE: {mae:.4f}")

# ------------------------------------------
# C. STRATÉGIE INCERTITUDE (MC DROPOUT)
# ------------------------------------------
print("\n>>> Évaluation INCERTITUDE (MC Dropout)")
mc_mae_history = []
df_pool = df_train_final.copy()
df_labeled = pd.DataFrame()
...
# Tracés Incertitude
plt.plot(steps, contrast_mae_history, marker='s', color='green', label='Active Learning (Incertitude Contraste)', linewidth=2)
plt.plot(steps, mc_mae_history, marker='^', color='orange', label='Active Learning (Incertitude MC Dropout)', linewidth=2)

plt.title("Active Learning : Évolution de l'Erreur (MAE) selon les Stratégies de Sélection", fontsize=14)
...
print("\nSimulation terminée avec succès ! Le graphique résume toutes vos stratégies.")